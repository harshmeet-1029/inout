<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT MEM_NO,MEM_NAME,Fname,Mname,MEM_EMAIL,MEM_TELEPHONE,MEM_ADM_DATE,MEM_CLOSE_DATE,GRP_NAME,DESIG_NAME,branchcode FROM gnestu WHERE MEM_NO='$usn' AND MEM_CLOSE_DATE > '$date'";
        $result = mysqli_query($koha, $sql) or die("Invalid query: 2" . mysqli_error());
        $data1 = mysqli_fetch_row($result);
        //image fetching
        $sql = "SELECT imagefile FROM patronimage WHERE MEM_NO = '$data1[0]'";
        $result = mysqli_query($koha, $sql);
        $data2 = mysqli_fetch_row($result);
        //Patron category code fetching
        $sql = "SELECT description FROM categories WHERE categorycode = '$data1[9]'";
        $result = mysqli_query($koha, $sql);
        $data3 = mysqli_fetch_row($result);
        //Branch information fetching
        $sql = "SELECT branchname FROM branches WHERE branchcode = '$data1[10]'";
        $result = mysqli_query($koha, $sql);
            ...
            if ($exit) {
                ...
                if (!$chk3) {
                    ...
                    if($chk4['loc'] != $_SESSION['locname']){
                        ...
                        $sql = "INSERT INTO `inout` (`sl`, `cardnumber`, `name`, `gender`, `date`, `entry`, `exit`, `status`,`loc`,`cc`,`branch`,`sort1`,`sort2`,`email`,`mob`) VALUES ('$sl', '$usn', '$data1[1]','', '$date', '$time', '".$_SESSION['libtime']."', 'IN','$loc','$data3[0]','$data4[0]','','','$data1[4]','$data1[5]');";
